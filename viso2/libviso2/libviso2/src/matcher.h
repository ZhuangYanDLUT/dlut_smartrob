...
public:
  ...
  struct p_match {
    ...
    bool operator < (const p_match &m) const
    {
        double d = max(u1p-u2p,0.00001f);
        double z = 582.508667*0.070103/d;
        double d_m = max(m.u1p-m.u2p,0.00001f);
        double z_m = 582.508667*0.070103/d_m;
        return z<z_m;
    }
  };
  ...
  std::vector<Matcher::p_match> getMatches() { return p_matched_2; }
	void setMatches(std::vector<Matcher::p_match> p_matched) { p_matched_2 = p_matched; }
